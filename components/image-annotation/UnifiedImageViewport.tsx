...
export function UnifiedImageViewport({
    ...
    if (existingParts && existingParts.length > 0) {
      ...
      if (parts.length === 0 || parts.length !== existingParts.length) {
        ...
        const mergedParts = existingParts.map((newPart) => {
          ...
          return existingPart
            ? { ...newPart, x: existingPart.x, y: existingPart.y }
            : newPart
        })
  ...
  const handleAddPartFromTemplate = useCallback(
    (template: any) => {
  ...
  const handlePartClick = useCallback(
    (e: React.MouseEvent | null, partId: string) => {
      if (e) {
        e.stopPropagation()
      }
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent) => {
      ...
      const updatedParts = parts.map((part) =>
        part.id === selectedPartId ? { ...part, x: newRelX, y: newRelY } : part
      )
  ...
  const handleUpdatePart = useCallback(
    (partId: string, updates: Partial<PartNode>) => {
  ...
  // Group parts
  const handleGroupParts = useCallback(
    (partIds: string[], groupId: string) => {
      console.log('🔗 Grouping parts:', {
        partIds,
        groupId,
        currentParts: parts,
      })
  ...
  // Ungroup part
  const handleUngroupPart = useCallback(
    (partId: string) => {
      const updatedParts = parts.map((p) =>
  ...
  // Bulk update parts
  const handleBulkUpdateParts = useCallback(
    (updates: Partial<PartNode>, selectedIds: string[]) => {
      const updatedParts = parts.map((p) =>
  ...
  // Bulk delete parts
  const handleBulkDeleteParts = useCallback(
    (partIds: string[]) => {
                ...
                <div
                  ...
                  className={`absolute w-6 h-6 rounded-full cursor-pointer border-2 transition-all duration-200 hover:scale-110 group ${
                    selectedPartId === part.id
                      ? 'border-yellow-400 shadow-lg animate-pulse'
                      : 'border-white'
                  } ${part.groupId ? 'ring-2 ring-purple-300' : ''} ${
                    selectedPartIds.includes(part.id)
                      ? 'ring-2 ring-green-400 bg-green-500'
                      : ''
                  }`}
                  style={{
                    left: absPos.x - 12, // Center the 12x12 dot
                    top: absPos.y - 12,
                    backgroundColor: selectedPartIds.includes(part.id)
                      ? '#10b981'
                      : part.color || '#3b82f6',
                  }}
    ...
    <Card className={className}>
          ...
          <div>
            ...
            <CardDescription>
              ...
              {currentState === 'annotation' &&
                'Click and drag parts to position them'}
            </CardDescription>
      ...
      <CardContent>
            ...
            <div className="space-y-2">
              {groups.map((group) => {
                const groupParts = parts.filter(
                  (part) => part.groupId === group.id
                )
                return (
                  <div
                    key={group.id}
                    className="flex items-center justify-between text-sm p-2 rounded bg-white border border-purple-100"
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-purple-500">
                        {groupParts.length} part
                        {groupParts.length !== 1 ? 's' : ''}
                      </span>
                    </div>
                      ...
                      {part.groupId && partGroup ? (
                        <div className="flex items-center gap-1">
                          <div
                            ...
                            style={{
                              backgroundColor: partGroup.color || '#8b5cf6',
                            }}
                          />
        ...
        <BulkPartEditor
          ...
          className={
            showBulkEditor
              ? 'fixed right-0 top-0 h-full w-96 bg-white shadow-2xl border-l border-gray-200 z-50'
              : 'hidden'
          }
        />
      </CardContent>
