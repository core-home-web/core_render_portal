...
export function DraggableElement({
    ...
    (e: MouseEvent) => {
      if (isDragging) {
        ...
        const newX = Math.max(
          0,
          Math.min(
            100 - (width / canvasWidth) * 100,
            originalPosition.x + (totalDeltaX / canvasWidth) * 100
          )
        )
        const newY = Math.max(
          0,
          Math.min(
            100 - (height / canvasHeight) * 100,
            originalPosition.y + (totalDeltaY / canvasHeight) * 100
          )
        )
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 z-10" onMouseDown={handleMouseDown} />
      {children}
